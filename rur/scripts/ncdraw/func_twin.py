...
def draw_sfms(ax, **kwargs):
    ...
    gals1 = kwargs['gals1']
    gals2 = kwargs['gals2']
    format_ax = kwargs.get('format_ax', True)
    colors = kwargs.get('colors', (P.COLOR1A, P.COLOR1B, P.COLOR1C))
    emarker = kwargs.get('emarker','o')
    c1,c2,c3 = colors
    ...
    yvals = gals1[ykey]
    if P.SHOW_SCATTER:
        ax.scatter(xvals, yvals, ec=c1, fc='none', s=20, zorder=1, lw=0.5)
        if P.SHOW_OTHER: ax.scatter(gals2['m'], gals2[ykey], fc='none', ec=c2, s=20, zorder=-1, lw=0.5)
    ...
    ax.errorbar(
        ...
        marker=emarker, color=c3, zorder=2, markersize=5,
        elinewidth=1, capsize=3, capthick=1
        )
    ax.plot(mbins, ybins[1], color=c3, lw=1, ls='--', zorder=2)
...
def draw_size(ax, **kwargs):
    ...
    if P.SHOW_SCATTER:
        ax.scatter(xvals, yvals, ec=c1, fc='none', s=20, zorder=1, lw=0.5)
        if P.SHOW_OTHER: ax.scatter(gals2['m'], gals2[ykey]/snap.unit[runit], fc='none', ec=c2, s=20, zorder=-1, lw=0.5)
...
def draw_cold(ax, **kwargs):
    ...
    if P.SHOW_SCATTER:
        ax.scatter(xvals, yvals, ec=c1, fc='none', s=20, zorder=1, lw=0.5)
        if P.SHOW_OTHER: ax.scatter(gals2['m'], gals2[ykey]/gals2['m'], fc='none', ec=c2, s=20, zorder=-1, lw=0.5)
...
def draw_metal(ax, key='Metal', **kwargs):
    ...
    if P.SHOW_SCATTER:
        ax.scatter(xvals, yvals, ec=c1, fc='none', s=20, zorder=1, lw=0.5)
        if P.SHOW_OTHER: ax.scatter(gals2['m'], get_metal(gals2,ykey)[0], fc='none', ec=c2, s=20, zorder=-1, lw=0.5)
...
def draw_mbh(ax, **kwargs):
    ...
    gals2 = kwargs['gals2']
    snap = kwargs['snap']
    if len(np.unique(gals1['MBH'])):
        gals1 = get_mbh(snap, gals1)
        gals2 = get_mbh(snap, gals2)
    format_ax = kwargs.get('format_ax', True)
    colors = kwargs.get('colors', (P.COLOR1A, P.COLOR1B, P.COLOR1C))
    emarker = kwargs.get('emarker','o')
    c1,c2,c3 = colors
    ...
    if P.SHOW_SCATTER:
        ax.scatter(xvals, yvals, ec=c1, fc='none', s=20, zorder=1, lw=0.5)
        ax.scatter(xvals[mask0], yvals[mask0], fc=c1, ec='none', s=20, zorder=1.5, lw=0.5)
        if P.SHOW_OTHER: ax.scatter(gals2['m'], gals2[ykey], fc='none', ec=c2, s=20, zorder=-1, lw=0.5)
...
def draw_shmr(ax, hals1, gids, **kwargs):
    ...
    if P.SHOW_SCATTER:
        ...
        if P.SHOW_OTHER:
            ...
            ax.scatter(hals1['mvir'][mask2], gals[gids[mask2]-1]['m']/hals1['mvir'][mask2], fc='none', ec=c2, s=20, zorder=-1, lw=0.5)

    CLUSTER = hals1[hals1['mvir']>=Msep]
    cxvals = CLUSTER['mvir']
    cyvals = gals[gids[hals1['mvir']>=Msep]-1]['m'] / cxvals
    ax.scatter(cxvals, cyvals, fc=c1, ec=c1, marker=emarker, s=50, zorder=4)
...
def draw_DTM(ax, **kwargs):
    ...
    if P.SHOW_SCATTER:
        ax.scatter(xvals, yvals, ec=c1, fc='none', s=20, zorder=1, lw=0.5)
        if P.SHOW_OTHER: ax.scatter(gals2['m'], get_DTM(gals2), fc='none', ec=c2, s=20, zorder=-1, lw=0.5)
...
def draw_CMD(ax, **kwargs):
    ...
    if P.SHOW_SCATTER:
        ax.scatter(xvals, yvals, ec=c1, fc='none', s=20, zorder=1, lw=0.5)
        if P.SHOW_OTHER: ax.scatter(gals2[f"{band}mag"], gals2[f"{band1}mag"] - gals2[f"{band2}mag"], fc='none', ec=c2, s=20, zorder=-1, lw=0.5)
    ...
    xe = 0.5*(xe[1:]+xe[:-1])
    ye = 0.5*(ye[1:]+ye[:-1])
    con = ax.contour(xe, ye, hist.T, levels=levels, colors=c3, alpha=0.7, zorder=3, linewidths=[0.5,1,1.5])
...
def draw_SB(ax, **kwargs):
    ...
    if P.SHOW_SCATTER:
        ax.scatter(xvals, yvals, ec=c1, fc='none', s=20, zorder=1, lw=0.5)
        if P.SHOW_OTHER: ax.scatter(gals2[f"{band}mag"], gals2[f"SB{band}_r50"], fc='none', ec=c2, s=20, zorder=-1, lw=0.5)
...
def draw_massfunc(ax, gals, hals, volume, **kwargs):
    global P
    ...
    zeromask = hmf>0
    ax.plot(hx[zeromask], hmf[zeromask], lw=1, color=c1, ls='--', zorder=0)

    gmf = np.histogram(np.log10(gals['m']), bins=gbins)[0] # number
    gmf = gmf / volume / (gbins[1] - gbins[0]) # number density
    zeromask = gmf>0
    ax.plot(gx[zeromask], gmf[zeromask], lw=1, color=c1, ls='-', zorder=0)
...
def draw_alpha(ax, **kwargs):
    ...
    if P.SHOW_SCATTER:
        ax.scatter(xvals, yvals, ec=c1, fc='none', s=20, zorder=1, lw=0.5)
        if P.SHOW_OTHER: ax.scatter(get_FeH(gals2), get_aFe(gals2), fc='none', ec=c2, s=20, zorder=-1, lw=0.5)
