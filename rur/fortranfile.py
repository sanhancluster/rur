...
class FortranFile(object):
    ...
    def _read_size(self):
        readed = np.fromfile(self._fp, dtype=self._header_dtype, count=1)
        if readed.size == 0:
    ...
    def read_arrays(self, read_num, dtype='i4'):
        """
        ...
        """
        array = []
        for _ in np.arange(read_num):
            array.append(self.read_record(dtype))
    ...
    def is_end(self):
        #print(chunk)
        end = self._fp.tell() == os.fstat(self._fp.fileno()).st_size
        #print(self._fp.tell(), os.fstat(self._fp.fileno()).st_size)
        return end
